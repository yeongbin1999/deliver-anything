...
public class DeliveryStatusSsePublisher {
  ...
  public void publish(DeliveryStatusEvent event) {
    if (event.customerProfileId() != null) {
      emitterRepository.sendToAll(event.customerProfileId(), EVENT_NAME, event);
    }
    if (event.sellerProfileId() != null) {
      emitterRepository.sendToAll(event.sellerProfileId(), EVENT_NAME, event);
    }
    if (event.riderProfileId() != null) {
      emitterRepository.sendToAll(event.riderProfileId(), EVENT_NAME, event);
    }
  }
}
